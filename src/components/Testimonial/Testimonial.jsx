...
const Testimonial = ({
  ...
  const SliderComponent =
    typeof window === "undefined" || typeof Slider !== "function"
      ? StaticSlider
      : Slider;
  ...
  const mergedTestimonials = defaultTestimonials.map((item, index) => ({
    ...item,
    ...(testimonials?.[index] || {}),
  }));
      ...
      >
        <div className="relative pt-3 pb-3 text-center border-b border-gray-100 px-4">
          <div className="absolute top-1.5 right-3 flex items-center gap-1 bg-[#efffe2] px-1.5 py-0.5 rounded-full border border-[#d4f5b8]">
            ...
            />
            <span className="text-[8px] font-bold text-[#00af87] uppercase tracking-wider">
              {providerLabel}
            </span>
          </div>

          <h4 className="text-[16px] md:text-[28px] font-black text-gray-900 tracking-tighter mb-0.5 md:mb-2 font-heading">
            {ratingLabel}
          </h4>
          ...
          <div className="flex flex-col items-center gap-1">
            <div className="flex items-center space-x-1">
              {[1, 2, 3, 4, 5].map((_, i) => (
                <div
                  key={i}
                  className="bg-[#00af87] p-1 rounded-full shadow-sm"
                >
            ...
            <p className="text-[10px] text-gray-500 font-medium font-sans">
              <span className="text-gray-900 font-bold underline decoration-safari-green decoration-1">
                {reviewCountLabel}
              </span>
            </p>
          </div>
        ...
        <div className="pb-3 pt-1">
          <SliderComponent {...settings}>
            {mergedTestimonials.map((review, index) => (
              <div
                key={review.id || `${review.name}-${index}`}
                className="outline-none px-4 md:px-6 py-2"
              >
                <div className="flex flex-row items-center text-left gap-3">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="flex justify-between items-center mb-0.5">
                      <h4 className="text-xs md:text-base text-gray-900 font-bold font-heading truncate">
                        {review.name}
                      </h4>
                      <div className="text-[8px] md:text-[10px] text-gray-400 font-bold uppercase">
                        {review.date}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 text-[#00af87] text-[10px] md:text-xs mb-1">
                      ...
                      <FaStar />
                      <FaStar />
                      <span className="ml-1 text-[8px] md:text-[9px] bg-green-50 text-green-700 px-1 py-0.5 rounded font-bold uppercase">
                        Verified
                      </span>
                    </div>
