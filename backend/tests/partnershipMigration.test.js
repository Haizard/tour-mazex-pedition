...
test("Partnership PostgreSQL-First Migration Test", async (t) => {
  try {
    ...
    if (shouldSkipForNetwork(error)) {
      t.diagnostic("Skipping full partnership migration integration due to Mongo network availability");
      return;
    }
  ...
  try {
    ...
    if (shouldSkipForNetwork(error)) {
      t.diagnostic("Skipping full partnership migration integration due to PostgreSQL network availability");
      return;
    }
