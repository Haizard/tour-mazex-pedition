...
export const normalizePrimaryAccommodationRows = (rows = []) =>
  rows.map((row = {}) => ({
    ...
    travelerEmail: String(row.source_payload?.travelerEmail || ""),
    travelerPhone: String(row.source_payload?.travelerPhone || ""),
    ...
    roomTypeCode: String(row.source_payload?.roomTypeCode || ""),
    units: toNumber(row.source_payload?.units, 1),
    checkInDate: toIso(row.check_in_date),
    checkOutDate: toIso(row.check_out_date),
    guestCount: toNumber(row.guest_count, 1),
    status: String(row.status || "pending"),
    paymentId: row.source_payload?.paymentId ? String(row.source_payload.paymentId) : "",
    paymentStatus: String(row.source_payload?.paymentStatus || "not-started"),
    sourceChannel: String(row.source_payload?.sourceChannel || "manual"),
    hotelIntentType: String(row.source_payload?.hotelIntentType || ""),
    pricing: row.source_payload?.pricing || {},
