...
| 18 | Guide & Driver Management | Partial | Tenant admins can now create guides and drivers, track availability, capture specialties and license details, assign field staff to bookings, enforce single-guide and single-driver coverage, and manage a live dispatch board with assignment windows. It still needs historical scheduling, calendar views, and outbound crew notifications. |
| 19 | Accommodation Coordination | Partial | Tenant admins can now coordinate hotel reservations, supplier contacts, room plans, guest counts, booking-linked stay windows, supplier-ready message drafts, and a live lodging board with stay-conflict detection. It still needs itinerary-level night-by-night automation and outbound supplier delivery. |
| 20 | Airport Pickup Coordination | Partial | Tenant admins can now schedule airport transfers, assign drivers, track airport codes and flight details, capture dispatch notes, generate driver-ready dispatch briefs, and manage a live arrival board with driver conflict detection. It still needs outbound notifications and true flight-status monitoring. |
| 21 | Payment Automation | Partial | Tenant admins can now create Stripe and Pesapal payment flows linked to bookings, track collection status, generate customer-facing checkout links, and calculate transaction fees in a dedicated revenue workspace. Live gateway credentials, webhooks, and provider-native checkout completion are still integration-ready rather than fully live. |
| 22 | Dynamic Pricing Engine | Partial | Enterprise tenants can now create dynamic pricing rules with season, demand, and occupancy multipliers, minimum floors, final price previews, and live package impact matching against tour inventory in a revenue workspace. It still needs public booking-flow application and automated runtime pricing updates. |
